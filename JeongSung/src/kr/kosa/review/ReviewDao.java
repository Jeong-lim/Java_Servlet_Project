...
public class ReviewDao {
	...
	//리뷰어 번호(reviewNumber) 찾는 메서드
	public int getReviewNumber(String name) {
		Connection con = null;
		int result=0;
		try {
			con = dataSource.getConnection();
			String sql = "select review_number from reviews where reviewer_name=?";
			PreparedStatement stmt = con.prepareStatement(sql);
		...
		System.out.println("메서드 안에 reviewNuM"+result);
		return result;
	}
	...
	//리뷰어 이름 가져오는 메서드
		public List<String> ReviewerSelects() {
			List<String> reviewerNameList = new ArrayList();
			Connection con = null;
			String result ="";
			try {
				con = dataSource.getConnection();
				String sql = "select DISTINCT reviewer_name from reviews";
				PreparedStatement stmt = con.prepareStatement(sql);
				ResultSet rs = stmt.executeQuery();
				while(rs.next()) {										
	...
	//읽기 Read (리뷰 글 조회)
		public ReviewVo getReview(int reviewNumber){
			ReviewVo data = new ReviewVo();
			Connection con = null;
			try {
				con = dataSource.getConnection();
				String sql = "select * from reviews where review_number=?";
				PreparedStatement stmt = con.prepareStatement(sql);
	...
	public List<ReviewVo> getAllReviews(){
		...
		try {
			con = dataSource.getConnection();
			String sql = "select * from reviews ORDER BY review_number ASC";
			PreparedStatement stmt = con.prepareStatement(sql);
			ResultSet rs = stmt.executeQuery();
			while(rs.next()) {
				ReviewVo review = new ReviewVo();
